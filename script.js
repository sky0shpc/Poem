...
function createPoemCard(poem, index) {
  ...
  sealBtn.setAttribute("aria-label", "Open this letter");
  ...
  hint.textContent = "unfold";

  sealBtn.appendChild(seal);
  sealBtn.appendChild(hint);

  head.appendChild(badge);
  head.appendChild(sealBtn);

  const preview = document.createElement("p");
  preview.className = "poem-preview";
  preview.textContent = firstLine(poem.text);
  ...
  sealBtn.addEventListener("click", () => toggleCard(card, sealBtn, hint));
  sealBtn.addEventListener("keydown", (e) => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      toggleCard(card, sealBtn, hint);
    }
  });
...
// Expand / collapse
// ==========================================================================
function toggleCard(card, sealBtn, hint) {
  const isExpanded = card.classList.contains("expanded");

  if (isExpanded) {
    collapseCard(card, sealBtn, hint);
  } else {
    card.classList.add("expanded");
    card.classList.remove("card-floating");
    sealBtn.setAttribute("aria-expanded", "true");
    sealBtn.setAttribute("aria-label", "Close this letter");
    hint.textContent = "fold closed";
  }
}

function collapseCard(card, sealBtn, hint) {
...
// ==========================================================================
// Hidden visit counter (Ctrl+Shift+V to reveal)
// ==========================================================================
const COUNTER_URL = "https://api.counterapi.dev/v1/YOUR_NAMESPACE/ranim-poems/up"; // <-- replace YOUR_NAMESPACE

function trackVisit() {
  fetch(COUNTER_URL).catch(() => {}); // fire and forget, fails silently
}
  ...
  document.addEventListener("keydown", (e) => {
    if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === "v") {
      fetch(COUNTER_URL.replace("/up", ""))
        .then((res) => res.json())
        .then((data) => alert("Total visits: " + data.count))
        .catch(() => alert("Could not load counter"));
    }
  });
}
